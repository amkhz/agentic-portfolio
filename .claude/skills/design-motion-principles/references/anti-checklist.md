...
**Flag when you see:** `box-shadow` / `filter: drop-shadow()` in an accent hue (`--theme-accent-*`) on hover/focus/active; any looped glow on interactive elements.
**Exception:** slow ambient "glow-from-within breathing" on tech-in-material *accents* is permitted vocabulary — a material treatment, not interaction feedback. Distinguish the two.

### Fixed cubic-bezier as the reflex
A bare `cubic-bezier()` or `ease` / `ease-in-out` used as the default driver for arrival or interaction. The mandate is spring/wave-driven; fixed beziers are reserved for trivial color/opacity crossfades.
**Flag when you see:** `transition: … cubic-bezier(…)` or `ease-*` on transform-bearing arrival/interaction where a token (`--ease-settle` / `--ease-organic` / `--ease-spring` / `--ease-out-expo`) should drive it. Point the fix at the token.

### Overshoot on arrival / focus
Bounce or elastic on section reveals, page transitions, parallax. Arrival is **critically damped** — `bounce: 0`. Overshoot is allowed ONLY on hover/focus/tap micro-interactions, and only small.
**Flag when you see:** `type: 'spring'` with `bounce > 0`, or a `cubic-bezier` with an overshoot peak > 1, on a reveal / transition / parallax.

### Performative / look-at-me motion
Beat-synced micro-interactions, scroll-jacking, motion that announces itself as design. Doctrine: "motion that announces itself as design has failed." Most things still; one ambitious motion moment per major surface, no more.
**Flag when you see:** ≥2 ambitious motion moments competing on one surface; scroll-hijack / pinned narrative on portfolio surfaces; motion whose only job is to be noticed.

### Animating layout properties
`width` / `height` / `top` / `left` / `margin` / `padding` in motion. Transform / opacity / filter only. (General elsewhere; a hard flag here.)
