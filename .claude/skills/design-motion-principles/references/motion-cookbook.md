...
| `--ease-default` / `--ease-in` / `--ease-out` | Material `cubic-bezier`s | short symmetric micro-transitions, theme/color crossfades |

The `linear()` bodies are long — reference the token name in code (`transition-timing-function: var(--ease-settle)`), never paste the sampled curve inline.

### Duration tokens
| Token | Value | Use |
|-------|-------|-----|
| `--duration-fast` | 100ms | button press, immediate feedback |
| `--duration-normal` | 200ms | standard state change |
| `--duration-slow` | 400ms | transitions with presence |
| `--duration-slower` | 600ms | arrival choreography |
...
```css
.reveal   { transition: opacity   var(--duration-reveal) var(--ease-settle),
