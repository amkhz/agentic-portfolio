...
The pipeline: invest-crew (scope) -> Tyrell (build) -> Roy (review) -> Impeccable skills (targeted fixes). When a task clearly fits one crew member's lane, suggest invoking that skill. The crew also uses the Impeccable design skill suite (v4) for design quality work: describe new-surface work in natural language (the deprecated `/impeccable craft` routes there anyway), `/impeccable critique` for scored dual-agent review (its persisted snapshot becomes `/impeccable polish`'s backlog), `/impeccable audit`, `/impeccable typeset`, `/impeccable doctor` for artifact drift, `/impeccable live` for in-browser variants, and the rest of the command table in `.claude/skills/impeccable/SKILL.md`. Every command is a sub-command of the one `impeccable` skill; the standalone 2.x command skills (`/polish`, `/audit`, ...) were removed.
