...
3. **Accessibility is non-negotiable** -- WCAG 2.2 AA throughout, both modes. One `h1` per page, heading hierarchy `h2 -> h3` in order, never skip levels. Three-face typographic system -- display, body sans, mono kicker. Variable axes used intentionally for *fit* (weight, optical size, grade), not for animation. Faces are not yet locked and are validated live before locking. No additional faces beyond the three. See `DESIGN.md` for full type direction.
...
- **Hard:** WCAG 2.2 AA compliance, both modes. All color is OKLCH via tokens (`design-system/tokens.css`) referenced by name -- no hex, no `rgb()`, no named colors anywhere. No em-dashes in copy. Three-face typographic system (display, body sans, mono kicker); faces validated live before locking, per `DESIGN.md` (ADR-013). Variable axes used intentionally for fit. No additional faces beyond the three.
