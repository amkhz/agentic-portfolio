...
1. **"The Conservatory"** -- An inhabited biophilic-future register: technology, nature, and people in long-settled, optimistic commune, grounded by Danish mid-century craft and wabi-sabi material honesty. Warm humus-blacks, never cold. Aged brass/amber is technology-and-light and owns interaction; soft sage/moss green is the living primary expressed as atmosphere and material only; dusty magenta is a rare signal. Dual mode is literal (humus night / golden-hour day). Full direction in `DESIGN.md` (binds ADR-013); supersedes the prior brass+magenta direction.
...
3. **Accessibility is non-negotiable** -- WCAG 2.2 AA throughout, both modes. One `h1` per page, heading hierarchy `h2 -> h3` in order, never skip levels. Three-face typographic system -- display, body sans, mono kicker. Variable axes used intentionally for *fit* (weight), not for animation. Faces LOCKED v2 (2026-06-24, "Stack E"): Hedvig Letters Serif (display) / Figtree (body) / JetBrains Mono (kicker), validated live both modes. No additional faces beyond the three. See `DESIGN.md` for full type direction.
4. **Tasteful motion** -- Motion is a pillar of the visual system: restrained but expressive, and **wave-driven by mandate** -- spring physics and spring-sampled easing drive every animation (a damped spring is a decaying sine wave; that is the organic feel). Arrival/focus is critically damped (no bounce/elastic); micro-interactions may carry a small damped overshoot. It serves arrival and focus, never spectacle. Most things still; the rare moment earns its weight. Respects `prefers-reduced-motion`. See `DESIGN.md`.
5. **Architecture protects craft** -- The four-layer separation (design-system, core, services, src) exists so that design decisions, business logic, external integrations, and UI never bleed into each other.

## Constraints

- **Hard:** WCAG 2.2 AA compliance, both modes. All color is OKLCH via tokens (`design-system/tokens.css`) referenced by name -- no hex, no `rgb()`, no named colors anywhere. No em-dashes in copy. Three-face typographic system (display, body sans, mono kicker); faces LOCKED v2 to Stack E -- Hedvig Letters Serif / Figtree / JetBrains Mono (2026-06-24), per `DESIGN.md` (ADR-013). Variable axes used intentionally for fit. No additional faces beyond the three.
