...
That last prediction is the document's most consequential claim, because static sub-vacuum persistence is "allegedly forbidden" by the |Quantum Inequalities|, a theorem that "has only been verified for single-mode squeezed light." Marecki discovered that the sub-vacuum regions inside a Casimir cavity violate the theorem, and the document says several investigators have strong arguments that the theorem is simply wrong for these cases. Testing that is explicitly part of the program: "We hope that experimental attempts to verify his predictions will follow."
