...
When the atoms scattered, they didn't bounce like billiard balls. Instead, each atom took *multiple paths simultaneously* — a core quantum behavior called superposition. The atoms' momenta became entangled: measuring the momentum of one atom instantly determined the momentum of its partner.
...
**Why it matters.** Previous entanglement experiments used photons (massless) or internal properties like spin (which don't interact with gravity). **The ANU team entangled the physical motion of atoms that have mass.** Because they have mass, they feel gravity. This gives experimentalists a handle — for the first time — to probe how quantum entanglement behaves in a gravitational field.

The experiment works *within* the standard framework. Quantum mechanics and general relativity are treated as separate theories. The goal is to create experimental conditions where both are relevant simultaneously, so we can observe what happens at the boundary. It's empirical — start with what we can measure and let the data guide theory.
...
Not approximately. **Exactly.** Every energy level, every spectral line, every orbital shape — all reproduced with zero free parameters beyond a single calibration to the known reduced mass of the electron-proton system.

**How the model works, in plain terms.** Imagine the vacuum as a vast ocean. A proton sitting in this ocean creates a disturbance — a density profile that falls off as you move away from it. Waves traveling through this medium near the proton experience different speeds depending on their distance from it — because the medium's properties vary with position. Now add one rule: short waves travel faster than long waves (quadratic dispersion, like waves in a shallow canal). With just those two ingredients — a proton-shaped density variation and this dispersion rule — the standing wave patterns that naturally form in this medium are *exactly* the orbitals of the hydrogen atom.
...
**The key move.** White doesn't reconcile quantum mechanics with gravity. **He proposes that quantization itself is an emergent property of the vacuum treated as a dispersive medium.** The quantum rules aren't fundamental — they're what you get when waves propagate through a medium with the right properties. If that's true, then "quantum" and "gravitational" might not be separate regimes at all. They'd both be features of the same underlying medium.

**The Puthoff connection.** This paper extends a research thread that traces directly back to Hal Puthoff's earlier work on stochastic electrodynamics. Puthoff showed in the 1980s and 1990s that the ground state of hydrogen can be understood as an equilibrium: the electron radiates energy, as any accelerating charge should, but simultaneously absorbs compensating energy from zero-point vacuum fluctuations. The orbit where radiation and absorption balance is exactly the known ground state. He later generalized this: *all* quantum ground states are equilibria between radiation and vacuum absorption.
...
The paper demonstrates *mathematical isospectrality* — the acoustic problem and the quantum problem produce identical eigenvalues and eigenfunctions. It does not claim to have unified quantum mechanics and general relativity. It does not claim the vacuum literally is a fluid. It does not make predictions that differ from standard quantum mechanics for hydrogen; the whole point is that it reproduces the standard results exactly.
