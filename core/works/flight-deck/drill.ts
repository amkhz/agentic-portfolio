 ...
 *
 * Adversarial inputs: the drill pushes the pure instrument models out
 * of band by feeding the same shapes out-of-band deltas (the models'
 * documented contract), not by a second model. The delta envelopes live
 * in drillEnvelopes.ts (split per Roy's phase-6 pre-read).
 */
