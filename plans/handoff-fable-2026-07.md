...
Justin asked for a paste-ready prompt per open work stream. Each is self-contained for a fresh session. Build state at writing: Selecta waves 1-3 + skill all MERGED (lastfm-mcp PRs #19-#26); T9 board on PR #27 with a glossary iteration in flight; T8 gated on Justin's first-ever Serato analysis run (library did not exist until today; 20k+ tracks analyzing).

**1. Selecta -- finish T9 (only if the glossary round didn't land):**
> Tyrell -- Selecta T9 glossary iteration was in flight on lastfm-mcp PR #27 (branch feat/set-board). Spec: a "What these terms mean" section at the board bottom, collapsed by default, zero-JS details-element pattern, defining exactly the terms the board surfaces (wheel steps, A/B swap, energy boost, key clash, halftime/doubletime, BPM/pitch fader, energy arc, mix window, tension and release), teaching register matching the camelot-primer voice. Check the PR for a follow-up commit past 32bfc2f; if present, regenerate/open the demo board for me and merge on my word. If absent, implement per spec, then same.

**2. Selecta -- T8 integration + my first set (the big one):**
...
**3. Aphelion / Nibiru -- shape session:**
...
**4. Mission 2 -- "Twenty Years of Listening" flagship shape:**
...
**5. lastfm-mcp -- backlog and the two gate issues:**
