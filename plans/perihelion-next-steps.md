...
- **PR #84 — B.2 voice sweep on `emergent-quantization` (Writer).** Third guide fully through both passes. Em-dashes 40 → 0 across 35 lines (PR #79's "36 lines" flag counted the Casimir entry, which B.1 itself had already de-dashed); 9 glossary entries trimmed to the 1–3 sentence rule (the six flagged plus three more 4-sentence violators caught in the pass), 12 punctuation-only de-dashes, "SQUARE" in `quadratic dispersion` lowercased; Design Hook added in `#so-what` ("The vacuum as a workable material", matching the dird-13 register); `dynamic vacuum` marker gap closed with a prose touch in `#vacuum` plus a new 2-sentence glossary entry — markers 38 → 39, all 38 pre-existing verified byte-identical. 102/102, 0 orphans. Mission log at `vector/missions/perihelion-b2-emergent-quantization.md`. Exit flag: the guide's frontmatter already carried `status: complete` entering B.2, so there was no draft flip to make; whether that stands or resets to the draft-until-merge convention is Justin's call at merge. In review.
- **PR #85 — B.1 sweep on `dird-14-superconductors-gravity` (fourth guide).** Glossary 45 in / 45 out (no dupes, no unused entries, no orphans; shared terms verified aligned against dird-15); 1 new `|term|` marker (`quantum coherence`, 46 resolved term nodes total); 3 inline glosses (gravity engineering, Marx generator, Rindler horizon); 7 callouts (Design Hook ×4, Read Next ×2, Territory Bridge ×1 — no Subguide queued since DIRD 19 is already flagged in quantum-gravity); `figures: []` removed; `source.authors` left untouched per the DIRD 14/28 deferral. 102/102, 0 orphans. Mission log at `vector/missions/perihelion-b1-dird-14-superconductors-gravity.md`. Exit flags for the B.2 pass: 62 em-dash lines in inherited copy; many glossary entries at 4–8 sentences vs the 1–3 rule; marker gaps needing prose touches (Chance Glenn never introduced in body, plural "Josephson junctions" blocks the exact-match marker, no `Hal Puthoff` glossary entry despite six possessive references); `status: draft` stands until B.2 per the dird-13 sequence. In review.
...
- **Pipeline continuation.** Next pair once PRs #84/#85 merge: B.2 voice sweep on `dird-14-superconductors-gravity` (Writer, scope per PR #85's exit flags above) + B.1 on the fifth guide — `dird-15-vacuum-spacetime-engineering` is the natural pick (sequential DIRD, and PR #85 already verified dird-14's shared terms against it). Operational notes, updated after the third pair: ad-hoc verification scripts are not allowlist-shaped — a heredoc `cat > /tmp/*.mjs` plus `node` / `npx tsx` attempts cost the dird-14 B.1 agent a three-denial detour before it self-corrected — so agent prompts should say validation stays on the npm scripts (`lint` / `build` / `test` / `audit:orphans`). The standing notes hold: generic worktree `node_modules` symlink rule in `.claude/settings.local.json`, `git commit -F <file>` for multiline messages, allowlisted single-purpose Bash throughout.
