...
- **Fifth pipeline pair + dird-14 fixes — in review (PRs #94 / #95 / #96).** Built overnight 2026-06-09 in three parallel worktrees; all four gates green on each (lint clean, build, 102/102, 0 orphans); guide PRs are Justin's merges, not orchestrator calls.
  - **PR #94 — B.1 on `dird-28-breakthrough-cockpits` (sixth guide, last DIRD).** Full format pass: 8 callouts added (Design Hook ×5 converting the closing design-implications section, Read Next ×2 to DIRDs 15 and 13, Subguide queued ×1 for DIRD 34; Territory Bridge ×0 as a documented deviation — no body prose draws a cross-territory line, surfaced for Justin), 3 inline glosses, `figures: []` removed; glossary arrived clean at 48 entries / 48 markers, zero additions needed, a series first. The deferred `source.authors` slot took the dead-end / institutional-fallback branch (`DIA / AAWSAP Program`, also closing dird-28's own AAWSA/AAWSAP mismatch); `vector/research/dird-authorship.md` updated. Exit flags for the dird-28 B.2: 51 em-dash lines (including the five verbatim Design Hook bodies); glossary majority at 3–6 sentences vs the 1–3 rule; second-person address in the `meditation chamber` and `TransDimensional Mapping` entries (invitational-posture flag); marker gaps (no `Hal Puthoff` entry despite possessive mentions, "spatial disorientation" precedes the `|Category A spatial disorientation|` marker, Birdie Jaworski identified only inside a glossary entry); declined gloss candidates `sensor fusion` and `progressive disclosure`. Mission log at `vector/missions/perihelion-b1-dird-28-breakthrough-cockpits.md`.
  - **PR #95 — dird-14 deferred fixes.** Justin's overnight authorization: `source.authors` → `DIA / AAWSAP Program` via the same dead-end branch (the research note explicitly records dird-14 as a dead end and the cover's author line is FOIA-redacted), which also closed the PR #85 AAWSA/AAWSAP mismatch; and the second `|Josephson junction|` marker in `#morningbird` un-marked to plain prose (first marker and glossary entry untouched). With #94, the `source.authors` deferral comes off the books entirely at merge: every DIRD now carries either a confirmed personal author (dird-15, Puthoff) or the canonical institutional fallback. One cosmetic wrinkle: the mission log (`vector/missions/perihelion-dird-14-source-fixes.md`) calls DIRD 28 "the only remaining deferred authors slot" — written before its sibling agent resolved dird-28 in parallel; harmless, fixable in the archive sweep.
  - **PR #96 — B.2 voice sweep on `dird-15-vacuum-spacetime-engineering` (Writer).** Fifth guide fully through both passes once merged. Em-dashes 112 → 0 across the 78 flagged lines (en-dashes 3 → 0); the five Design Hook bodies reworked in voice (first B.2 to touch callout copy; labels, markers, and structure intact); 64 of 65 glossary entries brought to the 1–3 sentence rule (only B.1's clean `Hal Puthoff` untouched); all three marker-gap flags closed via new `Chance Glenn`, `Martin Tajmar`, and `Ankur Bhatt` entries plus `#white-paper` now opening on `|Sonny White|'s 2026 paper`; markers 68 → 72 with all 68 pre-existing verified byte-identical. `status: draft` → `complete` flipped in-branch (`920518c`) per Justin's flip-after-the-voice-pass call. Leftovers: the DIRD 13 Read Next stays surfaced (dropped under the two-per-guide cap in B.1, untouched per scope); `Stochastic Electrodynamics` key casing kept per-guide, carried as a future normalization note; four terms now carry double markers (gravity, Morningbird Space, Casimir Inc., Sonny White), all resolving, un-markable in one line each if density reads heavy. Mission log at `vector/missions/perihelion-b2-dird-15-vacuum-spacetime-engineering.md`.
...
- **Pipeline continuation.** Next pair (sixth): B.2 voice sweep on `dird-28-breakthrough-cockpits` (Writer, scope per PR #94's exit flags above) + B.1 on one of the two remaining guides, `uap-field-map` or `uapx-field-methods` (the dird-28 log notes B.1 is now DIRD-complete but not library-complete). The closing passes on whichever guide remains then finish the eight. Operational notes, updated after the fifth pair: the orchestrator pwd-check caught cwd drift a third time (shell landed in a finished agent's worktree; absolute paths and `git -C` throughout); SendMessage to in-flight agents is not available in this harness, so mid-flight scope changes ride as post-completion commits on the agent's branch (the PR #96 status-flip pattern); the three agent worktrees stay in place until their PRs merge, then the usual branch/worktree cleanup applies. The standing notes hold: agents rename the worktree branch to the `feat/perihelion-...` pattern before pushing (all three did this round), generic worktree `node_modules` symlink rule in `.claude/settings.local.json`, `git commit -F <file>` for multiline messages, allowlisted single-purpose Bash throughout, validation on the npm scripts only (`lint` / `build` / `test` / `audit:orphans` — no ad-hoc verification scripts).
