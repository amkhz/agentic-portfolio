...
- **PR #89 — B.1 sweep on `dird-15-vacuum-spacetime-engineering` (fifth guide, Tyrell).** Glossary 64 → 65 (added `Hal Puthoff`, aligned with dird-13's entry; no dupes, no unused entries, no orphans); 3 new `|term|` markers (`Hal Puthoff` in `#big-picture`, `gravity` at its true first mention in `#sakharov-gravity`, `Morningbird Space` in `#lab-experiments`) for 68 resolved term nodes; 3 inline glosses (Extended Electrodynamics, UAP observable, PEAR lab); 8 callouts (Design Hook ×5 converted verbatim from the closing `#design-implications` bold-led paragraphs, Read Next ×2, Territory Bridge ×1 — no Subguide queued since DIRD 19 is already flagged in quantum-gravity); `figures: []` removed. `source.authors: Hal Puthoff` was already present in the file; confirmed against `vector/research/dird-authorship.md` and left standing, so the dird-15 authors slot is closed (only DIRDs 14 and 28 remain deferred). 102/102, 0 orphans; shared terms cross-checked from dird-15's side against dird-14, quantum-gravity, emergent-quantization, and dird-13 — aligned, nothing rewritten. Mission log at `vector/missions/perihelion-b1-dird-15-vacuum-spacetime-engineering.md`. Exit flags for the B.2 pass: 78 em-dash lines, including the five verbatim Design Hook bodies; the heaviest glossary in the library (65 entries, the majority at 4–8 sentences vs the 1–3 rule); marker gaps needing prose touches (Chance Glenn possessive-only again, `#white-paper` opens with "Harold White's" while the glossary key is `Sonny White`, Ankur Bhatt and Tajmar possessive/adjectival only); DIRD 13 has no Read Next despite three section references (dropped under the two-per-guide cap — surfaced for Justin, though quantum-gravity already pairs DIRDs 15 and 13); `source.url` missing (allowed per locked decision; body cites JBIS Vol. 63, 2010 if ever wanted); `Stochastic Electrodynamics` key casing differs from the lowercase siblings in quantum-gravity / emergent-quantization (per-guide keys, no contradiction, noted for any future normalization); `status: draft` stands until B.2. In review.
- **PR #90 — B.2 voice sweep on `dird-14-superconductors-gravity` (Writer).** Fourth guide fully through both passes. Em-dashes 95 characters across 62 lines → 0 (en-dashes 4 → 0, matching the prior B.2 zero baseline); glossary 45 → 46 (added `Hal Puthoff` in body-mention order), 37 entries trimmed from 4–7 sentences to the 1–3 rule (including all six B.1-flagged), 6 de-dashed without trimming; markers 46 → 48 (`Hal Puthoff` first-mention via the established `|term|'s` possessive pattern in `#big-picture`, `Josephson junction` via a singularized SQUIDs parenthetical), all 46 pre-existing markers verified byte-identical via `git diff --word-diff=porcelain`; Chance Glenn introduced in `#morningbird` body prose; all 3 definition glosses verbatim-synced through their trims. 102/102, 0 orphans. Mission log at `vector/missions/perihelion-b2-dird-14-superconductors-gravity.md`. Exit flags: `status: draft` flip to complete is Justin's call at merge (per the dird-13 sequence); a double `|Josephson junction|` marker now sits in one `#morningbird` paragraph (both resolve; un-marking the second is a one-line tweak that touches a pre-existing marker, left for Justin); the DIRD 14 `source.authors` deferral and the AAWSA/AAWSAP mismatch are unchanged; `source.url` missing (allowed); the non-schema `source.notes` field left in place. In review.
...
- **Pipeline continuation.** Next pair once PRs #89/#90 merge: B.2 voice sweep on `dird-15-vacuum-spacetime-engineering` (Writer, scope per PR #89's exit flags above — note the five verbatim Design Hook bodies carry em-dashes, so the sweep touches callout copy for the first time) + B.1 on the sixth guide — `dird-28-breakthrough-cockpits` is the natural pick (last DIRD in the library, still `status: draft`, and its deferred `source.authors` slot comes due with it), leaving `uap-field-map` and `uapx-field-methods` to close out the eight. Operational notes, updated after the fourth pair: agents should rename the worktree branch before pushing — PR #90 shipped from the default `worktree-agent-*` branch name (harmless, but PR #89's `feat/perihelion-b1-...` rename is the pattern); and the orchestrator pwd-check before branch work earned its keep again (cwd drifted into an agent worktree mid-session; caught with no damage). The standing notes hold: generic worktree `node_modules` symlink rule in `.claude/settings.local.json`, `git commit -F <file>` for multiline messages, allowlisted single-purpose Bash throughout, validation on the npm scripts only (`lint` / `build` / `test` / `audit:orphans` — no ad-hoc verification scripts).
- **DIRDs 14 and 28 `source.authors` slot — deferred.** PR #60 landed the safe spelling + ICOD pieces and intentionally left the authors slot untouched. PR #61's research note (`vector/research/dird-authorship.md`) is the home for grounding research. Decision tree: if research surfaces personal authors → use them; if it dead-ends → institutional `DIA / AAWSAP Program` ships as the canonical fallback. PR #85 additionally logged an `AAWSA` vs `AAWSAP` mismatch inside dird-14's source block, to resolve together with that decision. No urgency.
