...
- **Content leftovers surfaced for Justin (no urgency, none block the pipeline).** dird-28's Territory Bridge ×0 was blessed as-is on 2026-06-10: no body prose draws a genuine cross-territory line, so zero stands, carried only as an opportunistic watch item (if a future pass or the uapx-field-methods work surfaces a real bridge, it rides as a small content commit). Still open from the fifth pair: dird-15's DIRD 13 Read Next still dropped under the two-per-guide cap; `Stochastic Electrodynamics` glossary key casing kept per-guide, carried as a future normalization note; four dird-15 terms carry double markers (gravity, Morningbird Space, Casimir Inc., Sonny White), all resolving, un-markable in one line each if density reads heavy.
- **C.2 follow-up — curated `sectionIcons` frontmatter system. Reaffirmed 2026-06-09 (overnight).** Justin still wants the icon pass: it is part of what makes the library feel designer-authored, alongside the upcoming design tweaks. Sequenced after the content sweep (Workstream B) completes, and the approach gets revisited through the lens of the updated Impeccable skills (v3.5.0 reshape) before build. Mechanics unchanged: a frontmatter field mapping section anchors to icon names (Lucide for standard affordances, Phosphor where a pictorial register is wanted), rendered at the `<h2>` and the `GuideSectionNav` chip; still blocked on Justin's icon picks; the type-only interim state since PR #71 is the live baseline to beat.
- **PR #105 in review — B.2 voice sweep on `uap-field-map` (Writer, seventh pair).** All PR #100 exit flags closed: em-dashes 166 → 0 across the 136 inherited lines (en-dashes 36 → 0), all restructured in voice, with two in-quote dashes handled as transcription-punctuation comma swaps and the `{#framing-scientific-problem}` anchor kept byte-identical. Glossary 73 → 64 entries, 0 unused (was 15): five duplicate pairs consolidated into the marked keys (Residuum, Angel hair, PCM, STM, Project SIGN/Grudge/Blue Book), five deleted with per-entry reasoning in the mission log (AARO predecessors, Florence 1954, Forensic vs. observational paradigm, JANAP 146 / AFR 200-2, LPS); all entries now within the 1–3 sentence rule. Markers 75 → 81 nodes (58 → 64 keys): new `Hal Puthoff` entry + possessive marker per the dird-28/dird-15 pattern, plus light prose touches marking Galileo Project, IFEX, SETI Kingsland, VASCO, and Wendt-Duvall; all pre-existing markers verified byte-identical. Both dangling-colon content gaps restored from the design-futures upstream (`learning-guides/uap-field-map.md`, untouched): the IFEX VaMEx3-MarsSymphony pull-quote (upstream line 472, reworked em-dash-free) and the methodology cold-UAP datum (upstream line 700, verbatim). Second Read Next slot filled with DIRD 15 in the Pattern 3 AATIP→DIRD lineage subsection (callouts now 21 Design Hook / 17 Territory Bridge / 2 Read Next / 3 Subguide queued; the resulting callout stacking flagged as a watch item). INTERMAGNET gloss stays declined; no status flip (the guide entered and left `complete`). Gates green, 102/102, 0 orphans. Provenance: the first Writer-lane attempt stalled during recon (stream watchdog, 600s) with an empty diff; the orchestrator recovered its recon intel (the upstream payload locations) from the transcript via bounded greps, removed the dead worktree, and relaunched fresh with the intel in the prompt. Mission log at `vector/missions/perihelion-b2-uap-field-map.md` (rides on the PR branch; archive after merge).
- **Pipeline continuation.** Seventh (final) pair status: #104 merged, #105 in review above. Once #105 merges, one mission remains — the B.2 voice sweep on `uapx-field-methods` (scope = PR #104's eleven exit flags in Shipped above, full detail in `vector/missions/archive/perihelion-b1-uapx-field-methods.md`) — and the content sweep (Workstream B) closes. Operational notes, updated after the seventh pair: a stream-watchdog stall (no progress for 600s) can also hit during recon with zero edits made — then there is nothing to salvage; the orchestrator cleans up the dead worktree (unlock first if locked), recovers any useful recon intel from the transcript with bounded greps, and relaunches fresh with that intel in the prompt (the uap-field-map B.2 pattern). The post-edit stall recovery stands: the edit diff survives in the worktree; the orchestrator verifies it against scope with bounded greps, runs the gates, authors the mission log from the diff with a provenance note, and ships on the agent's branch. The standing notes hold: orchestrator pwd-check before any branch work (absolute paths and `git -C` throughout); SendMessage to in-flight agents is not available, so mid-flight scope changes ride as post-completion commits on the agent's branch; agents rename the worktree branch to the `feat/perihelion-...` pattern before pushing; generic worktree `node_modules` symlink rule in `.claude/settings.local.json`; `git commit -F <file>` for multiline messages; allowlisted single-purpose Bash throughout; validation on the npm scripts only (`lint` / `build` / `test` / `audit:orphans` — no ad-hoc verification scripts).
