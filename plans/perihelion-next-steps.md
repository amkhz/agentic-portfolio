...
- **PR #71 — C.2 emoji strip across the 7 remaining guides.** Executes path (a) of the emoji-to-icon decision (picked 2026-06-09). Sweeps all 54 leading emoji tokens from `## ` headers (every header carries an explicit `{#anchor}`, so anchors and nav links are unchanged); retires `splitLeadingIcon`, the `icon` field on `GuideSection` / `HeadingBlock`, and the icon render paths in `GuideSection.tsx` / `GuideHeading.tsx` / `GuideSectionNav.tsx`; adds a parser warn on `\p{Extended_Pictographic}` in headings to guard the upstream guide-intake convention. 102/102 tests. Ready for review.
...
- **PR #72 — B.1 sweep on `quantum-gravity` (starter guide, picked 2026-06-09).** First guide through the per-guide pipeline, stacked on PR #71. Full retrofit: 18-entry glossary (shared terms aligned with `emergent-quantization`), `|term|` markers, 3 inline definition glosses, 5 callouts (Design Hook conversion, Read Next ×2, Territory Bridge, Subguide queued for DIRD 19), `figures: []` removed. Zero orphans, zero plain fall-throughs, 102/102. Mission log at `vector/missions/perihelion-b1-quantum-gravity.md`. Exit flags: `kicker: "Synthesis Guide"` deviates from the two blessed kicker values; `source.url` missing (two papers, one slot). In review.
- **Pipeline continuation.** Behind PR #72: B.2 voice sweep on `quantum-gravity` (Writer, on the cleaned structure), then B.1 on the next guide — `dird-13-warp-drive` is the natural second. Tyrell aligns guide N+1 while Writer sweeps guide N. The pipeline only touches `core/lab/guides/*.md` prose, so it no longer collides with anything in C.2.
...
7. **C.2 — effectively closed pending PR #71 merge.** Shipped: GuideCard hover (PR #63), `uap-field-map.md` callout icons + emoji sweep (PR #64), parser tolerance removal (PR #65), restyling survey (PR #66), graphite recalibration (PR #67). Path (a) picked 2026-06-09 for the emoji question: the strip + `splitLeadingIcon` retirement is PR #71 (in review); the curated `sectionIcons` frontmatter follow-up waits on Justin's icon picks and may be dropped if the type-only interim reads better.
8. **Now in parallel — per-guide pipeline + C.2 follow-on:**
  - **Per-guide pipeline (B.1 alignment → B.2 voice)** — for each guide: callout retrofit, inline definition glosses, voice sweep. Pipeline two agents — Tyrell aligning guide N+1 while Writer sweeps guide N. Streams don't block. Pick a starter guide before kicking off. The emoji question is settled (path (a), PR #71), so the pipeline is unblocked.
