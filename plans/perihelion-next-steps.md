...
- **DIRD secondary frontmatter follow-ups.** Three small mechanical items surfaced by PR #51 that didn't fit the original scope: AAWSA-vs-AAWSAP spelling drift between source PDFs and the spec, `source.authors` drift on DIRDs 14 and 28 (currently carry the venue value), ICOD-vs-publication-date typo on the DIRD 14 cover. Ships as a small B.1 sub-pass — touches `core/lab/guides/*.md` only, no collision with anything else.
...
6. **Now in parallel — per-guide pipeline + C.2:**
  ...
  - **DIRD secondary frontmatter follow-ups** — small mechanical B.1 sub-pass (AAWSAP spelling, source.authors drift on 14/28, ICOD typo on 14). Can run alongside or ahead of the first per-guide sweep.
...
7. **Workstream D (logotype).** Anytime. Different hands, different surfaces.
8. **Workstream E (nested definitions).** Dreamer session whenever Justin wants to explore. Output is a plan, not code.
...
- **DIRD secondary frontmatter follow-ups** — surfaced by the Track A pass (PR #51, merged) and now promoted to the active queue: AAWSA-vs-AAWSAP spelling drift between source PDFs and the canonical spec; `source.authors` drift on DIRDs 14 and 28 (currently carry the venue value in the authors slot); ICOD-vs-publication-date typo on the DIRD 14 cover. Touches `core/lab/guides/*.md` only.
