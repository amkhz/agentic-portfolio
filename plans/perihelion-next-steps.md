...
- **C.2 second-pass PRs ready to merge.**
  - **PR #66 — C.2 lab-component restyling survey.** Read-only audit of six lab library components (`PerihelionSigil`, `TerritoryBadge`, `TerritoryGrid`, `LibraryHeader`, `LibraryWelcome`, `UpcomingCard`) against the C.1.5 graphite floor. Identifies which contrast / opacity-driven elements were calibrated against the prior darker `--lab-bg-deep` and likely feel softer on graphite; punch list with P2/P3 severity, candidate fixes (none requiring new tokens), recommended implementation order. Plan doc only, no code changes.
  - **PR #67 — C.2 graphite recalibration.** Three live-verified fixes against the C.1.5 floor, closing the active P2 items from PR #66's survey. LibraryHeader colophon affordance: text-glyph `+` → Lucide `Plus` icon at 14px in `guide-accent/80` (reads as interactive at rest, not just on hover). LibraryWelcome Field Notes cards: `border-t` → full `border` with `rounded-lg` (contained editorial units, deep→surface delta alone is too soft post-C.1.5). TerritoryBadge pulse: opacity range `[0.2, 0.32]` → `[0.32, 0.6]`, scale range `[1, 1.1]` → `[1, 1.3]`, settled `0.2` → `0.32` (pulse ring now extends ~6px past the 44px badge perimeter with a clearly visible halo). 101/101 tests pass.
- **C.2 — emoji-to-icon replacement across the remaining 7 guides — explicitly deferred 2026-05-25.** 54 emojis remain in section headers across `dird-13-warp-drive.md`, `dird-14-superconductors-gravity.md`, `dird-15-vacuum-spacetime-engineering.md`, `dird-28-breakthrough-cockpits.md`, `emergent-quantization.md`, `quantum-gravity.md`, and `uapx-field-methods.md`. All on `## ` lines (none on h3, none in callouts, none in body); currently rendered by the parser's `splitLeadingIcon` into `section.icon` and shown at both the `<h2>` and the `GuideSectionNav` chip. **Doctrine says replace, not strip** (this file, locked 2026-05-03 line 52: "C.2 sweeps existing guides into compliance" + line 152: "Lucide for standard UI affordances and Phosphor where a more opinionated pictorial register is wanted"). Three concrete paths surfaced this session: (a) split into two PRs — fast strip + retire `splitLeadingIcon` now, then a curated `sectionIcons` frontmatter system in a follow-up PR with Justin's icon-pick input; (b) one larger PR doing the full replacement (parser + frontmatter field + renderer + 54 curated picks all in one); (c) strip-and-decide-icons-are-gone-for-good, superseding the 2026-05-03 doctrine in favor of type-only editorial section openers. Picking one is the next open call.
- **Next mission — first per-guide B.1 sweep.** With the renderer landed, the per-guide pipeline can begin: callout retrofit (Design Hook / Territory Bridge / Read Next / Subguide queued where the prose supports), inline definition gloss injection for heavy first-mentions, then B.2 voice sweep behind it. Pick a starter guide (likely `dird-13-warp-drive` or `quantum-gravity`). Note: this pipeline only touches `core/lab/guides/*.md`, so it does not collide with the still-open emoji-to-icon C.2 path above — except that a per-guide B.1 sweep + an emoji-to-icon pass on the same guide are most efficiently bundled into one mission. Worth picking the emoji-to-icon path before the per-guide pipeline kicks off.
...
7. **C.2 first pass — partially shipped 2026-05-25.** Closed: GuideCard hover (PR #63), `uap-field-map.md` callout icons + emoji sweep (PR #64), parser tolerance removal (PR #65). Open and ready to merge: lab-component restyling survey (PR #66), graphite recalibration (PR #67). **Still open within C.2:** emoji-to-icon replacement across the remaining 7 guides (54 emojis, all `## ` lines, doctrine says replace not strip). Path-forward decision is the next open call — see "Open / in flight" above for the three concrete options.
8. **Now in parallel — per-guide pipeline + C.2 follow-on:**
  - **Per-guide pipeline (B.1 alignment → B.2 voice)** — for each guide: callout retrofit, inline definition glosses, voice sweep. Pipeline two agents — Tyrell aligning guide N+1 while Writer sweeps guide N. Streams don't block. Pick a starter guide before kicking off. Sequencing note: if the C.2 emoji-to-icon path is picked first, fold the per-guide B.1 sweep into the same per-guide mission (same files, less ceremony).
  - **C.2 follow-on items**: any P3 items from the survey that survive a second live review (sigil orbit dim end, section dividers — both were verified live as fine in the first pass, likely "won't fix"); the `splitLeadingIcon` retirement (only after the 7-guide emoji-to-icon path lands).
