...
- **dird-14 double `|Josephson junction|` marker.** PR #90 left one `#morningbird` paragraph with two resolving markers for the same term (both render fine; the duplicate came from the singularized SQUIDs parenthetical). Un-marking the second is a one-line tweak that touches a pre-existing marker, so it stays Justin's call; fold into any future dird-14 touch. No urgency.
...
- **Pipeline continuation.** Next pair (fifth): B.2 voice sweep on `dird-15-vacuum-spacetime-engineering` (Writer, scope per PR #89's exit flags, full detail in the archived mission log: 78 em-dash lines including the five verbatim Design Hook bodies, so the sweep touches callout copy for the first time; the heaviest glossary in the library at 65 entries, the majority at 4–8 sentences vs the 1–3 rule; marker gaps needing prose touches — Chance Glenn possessive-only, `#white-paper` opens "Harold White's" while the key is `Sonny White`, Ankur Bhatt and Tajmar possessive/adjectival only; the DIRD 13 Read Next dropped under the two-per-guide cap, surfaced for Justin; `Stochastic Electrodynamics` key casing noted) + B.1 on the sixth guide, `dird-28-breakthrough-cockpits` (last DIRD in the library, still `status: draft`; its deferred `source.authors` slot comes due — apply the decision tree in the deferral bullet below), leaving `uap-field-map` and `uapx-field-methods` to close out the eight. Operational notes, updated after the fourth pair: agents should rename the worktree branch before pushing — PR #90 shipped from the default `worktree-agent-*` branch name (harmless, but PR #89's `feat/perihelion-b1-...` rename is the pattern); and the orchestrator pwd-check before branch work earned its keep again (cwd drifted into an agent worktree mid-session; caught with no damage). The standing notes hold: generic worktree `node_modules` symlink rule in `.claude/settings.local.json`, `git commit -F <file>` for multiline messages, allowlisted single-purpose Bash throughout, validation on the npm scripts only (`lint` / `build` / `test` / `audit:orphans` — no ad-hoc verification scripts).
