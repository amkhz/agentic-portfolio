...
- **PR #74 — B.2 voice sweep on `quantum-gravity` (Writer).** First guide fully through both pipeline passes. 41 em-dashes restructured to 0, all 20 markers byte-identical, parser baseline from PR #72 held exactly. Mission log at `vector/missions/perihelion-b2-quantum-gravity.md`. Merged.
- **PR #75 — B.1 sweep on `dird-13-warp-drive` (second guide).** Glossary 44 → 45, 4 new markers, 3 glosses, 8 callouts, `figures: []` removed. Mission log at `vector/missions/perihelion-b1-dird-13-warp-drive.md`. Exit flags carried into PR #78. Merged.
- **PR #76 — plan resync, pipeline live.** Merged.

### Open / in flight

- **PR #78 — B.2 voice sweep on `dird-13-warp-drive` (Writer).** Second guide fully through both passes. Em-dashes 78 → 0 (the PR #75 exit flag's "~58" was a line count, not a character count); `#observables` heading text de-dashed with its anchor and all 8 others byte-identical; 39 of 45 glossary definition lines touched (33 trimmed to the 1–3 sentence rule including the three flagged offenders, 6 punctuation-only de-dashes); the three body glosses kept in verbatim sync with their trimmed frontmatter entries; all 47 `|term|` markers byte-identical; parser baseline from PR #75 holds exactly (8 callouts on variant, 3 glosses, 0 plain fall-throughs). 102/102, 0 orphans. Mission log at `vector/missions/perihelion-b2-dird-13-warp-drive.md`. Exit flag: `status: draft` — with both passes done, flipping to `complete` is Justin's call at merge (quantum-gravity and emergent-quantization already carry `complete`). In review.
- **PR #79 — B.1 sweep on `emergent-quantization` (third guide).** Lightest retrofit yet: the guide arrived nearly canonical (38-entry glossary with no dupes or gaps, full first-mention marker coverage, 0 markers added). `source.url` filled with the White paper canonical link lifted from quantum-gravity's `source.notes` (no `arxiv:` added — the recorded `2502.12392` belongs to the ANU paper); 3 inline glosses (Casimir effect, Coulomb problem, stochastic electrodynamics); 1 Read Next to quantum-gravity in `#so-what`; `figures: []` removed. Parser sweep clean, 102/102, 0 orphans. Mission log at `vector/missions/perihelion-b1-emergent-quantization.md`. Exit flags for the B.2 pass: 36 em-dash lines inherited in body/glossary; six glossary entries over the length rule plus a shouting-caps "SQUARE" in `quadratic dispersion`; first guide to exit B.1 with **no Design Hook** (the `#so-what` vacuum-engineering implication is the candidate, but it's a content add, so B.2's lane); `dynamic vacuum` marker gap — the title leads with the phrase but the body never uses it literally, so a B.2 prose touch is needed to create the marker opportunity. In review.
...
- **Pipeline continuation.** Next pair once PRs #78/#79 merge: B.2 voice sweep on `emergent-quantization` (Writer, scope per PR #79's exit flags — em-dash sweep, glossary trims, the Design Hook content add from `#so-what`, and the `dynamic vacuum` marker opportunity) + B.1 on the fourth guide — `dird-14-superconductors-gravity` is the natural pick. Operational notes, updated after the second pair: the worktree `node_modules` symlink rule in `.claude/settings.local.json` is now generic (no more per-agent-path entries — the old hardcoded ones caused the B.1 agent's 2-minute-timeout denial spiral); multiline `git commit -m` is not allowlist-shaped, so agents should write the message to a file and use `git commit -F`; keep Bash to allowlisted single-purpose commands throughout.
