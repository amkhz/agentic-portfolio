#!/bin/bash
# Agentic Portfolio setup — gets you from zero to running app

set -e

echo ""
echo "  Agentic Portfolio Setup"
...
if ! command -v node &> /dev/null; then
  ...
  if ! command -v node &> /dev/null; then
    ...
    echo "  Close this terminal, open a new one, and re-run bash scripts/install.sh"
    exit 0
  fi
else
...
echo "    .agents/skills/   — agent skill definitions"
echo "    vector/           — Zero Vector research schemas"
echo ""
echo "  Next steps:"
echo "    1. Run:  bash scripts/start.sh    — Launch dev server and Claude Code"
echo "    2. Run:  bash scripts/preflight.sh — Verify everything is ready"
echo ""
