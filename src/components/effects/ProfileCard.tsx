...
import { createTiltEngine, type TiltEngine } from "./tiltEngine";
...
export function ProfileCard({
  ...
  iconUrl,
  grainUrl,
  innerGradient = "linear-gradient(145deg, oklch(0.22 0.01 286 / 0.55) 0%, oklch(0.71 0.08 61 / 0.27) 100%)",
  behindGlowEnabled = true,
  behindGlowColor = "oklch(0.71 0.08 61 / 0.5)",
  behindGlowSize = "50%",
  aspectRatio = "1",
  className = "",
  enableTilt = true,
  ...
  const tiltEngine = useMemo<TiltEngine | null>(() => {
    ...
    return createTiltEngine(shellRef, wrapRef);
  }, [shouldTilt]);
  ...
  useEffect(() => {
    if (!shouldTilt || !tiltEngine) return;
    const shell = shellRef.current;
    if (!shell) return;
    ...
    const handleMobileTiltClick = (): void => {
    ...
    shell.addEventListener("click", handleMobileTiltClick);
    ...
    return () => {
      ...
      shell.removeEventListener("click", handleMobileTiltClick);
  ...
  const shineStyle: React.CSSProperties = {
    ...
    transform: `translate3d(0, 0, 1px)${iconAngle ? ` rotate(${iconAngle}deg)` : ""}`,
  ...
  return (
    ...
    >
      {behindGlowEnabled && (
        <div
          className="pointer-events-none absolute inset-0 z-0 transition-opacity duration-normal ease-out"
          style={{
      ...
      <div ref={shellRef} className="group relative z-[1]">
        <div
          ...
          style={{
            ...
            background: "var(--color-bg-deep, rgba(0, 0, 0, 0.9))",
        ...
        >
          <div
            ...
            style={{
              ...
              backgroundColor: "var(--color-bg-deep, rgba(0, 0, 0, 0.9))",
              borderRadius: cardRadius,
              gridArea: "1 / -1",
            }}
          >
            <div style={shineStyle} />
            {/* Grain noise layer — gives the glitchy sparkle between icon tiles */}
            <div
              style={{
            ...
            >
              <img
                className="absolute bottom-[-1px] left-1/2 w-full will-change-transform transition-transform duration-fast ease-out"
                src={avatarUrl}
                alt={avatarAlt}
                loading="lazy"
                style={{
