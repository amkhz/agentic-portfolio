...
/**
 * Perlin noise-driven wave grid background.
...
class PerlinNoise {
  ...
  seed(seed: number) {
    ...
    for (let i = 0; i < 256; i++) {
      const v = i & 1
        ? PerlinNoise.p[i] ^ (seed & 255)
        : PerlinNoise.p[i] ^ ((seed >> 8) & 255);
      this.perm[i] = this.perm[i + 256] = v;
      this.gradP[i] = this.gradP[i + 256] = PerlinNoise.grad3[v % 12];
    }
  }
...
interface WavesProps {
  /** Line color. Defaults to --theme-border-subtle. */
  lineColor?: string;
  waveSpeedX?: number;
  waveSpeedY?: number;
  waveAmpX?: number;
...
export function Waves({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener("resize", () => { setSize(); setLines(); });
      window.removeEventListener("mousemove", onMouseMove);
    };
  }, [lineColor, waveSpeedX, waveSpeedY, waveAmpX, waveAmpY, xGap, yGap, friction, tension, maxCursorMove, prefersReduced]);
