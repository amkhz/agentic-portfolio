...
- **Resolved (Justin, 2026-07-03): Works links from the Perihelion home by evolving the Shelf, not building a foyer.** The arms coexist on the current home: the Shelf gains a real Works section in a distinct register from the guide shelf (the Flight Deck's sigil + thesis line, an instrument on the shelf of books, not another book spine), with the masthead carrying the house identity and each arm's line per D7. A true foyer is deferred until Works 02 gives the arm enough mass to deserve a door. This touches Archive-side surfaces (LabLayout home), so it ships as its own small shape + PR, not inside the deck's phases; sequence it near ship, since the section needs the deck's sigil and thesis line to exist.
