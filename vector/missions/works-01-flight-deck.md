...
**Now:** Annotation layer BUILT 2026-07-04 (this PR): the locked "earn the width" composition is real. Caliper arcs hug the wall at the three tracked stress concentrations with hairline leaders to per-lane readouts (ST 1-3: bearing off 12 o'clock, intensity, WATCH/QUIET), anchored by inverting the shader's own transform from the same clock and pure field model, so arc and render cannot disagree; presence is aspect-driven across the same regime as the shader's anchor guard (no timed entrance), the sr mirror gained a sentence per lane, and the static plate and decline card are untouched. This pre-builds the pointable anatomy phases 4-5 need. Stack note: PR #178 had merged into its base branch only and never reached main; PR #179 relands that stack (DialKit tuner, frame dials, baked defaults) and this PR is stacked on it. Phase 2 context: MERGED 2026-07-03 (PR #175) with same-day motion pass, Justin-approved live; deck chunk 51 of 350 KB gz. Next up: **Justin's live gut-check on the annotation layer** (lane column position, arc weight, presence window all judged on the bench; tuner's frame dials still drive the anchor), then **Phase 3 — Synthetic Orientation + Vacuum Energy** (Roy reviews again at phase 5). Boot-feel tuning still open (Justin sleeping on it). Dev tuning: `dialkit@1.3.0` drives the field shader's 15 params as live dials (`src/works/flight-deck/dev/FieldTuner.tsx`); dev-only, absent from production; "log values" copies JSON to bake back into `FIELD_MOTION_DEFAULTS`.
...
- [x] **Annotation layer ("earn the width")** (built 2026-07-04, stacked on PR #179): caliper arcs + leader lines from the three tracked stress concentrations to per-lane readouts (`ST n` / bearing / intensity / watch state), projected in `fieldAnnotation.ts` by inverting the shader transform (frame dials, aspect guard, ring centerline) so annotation and render share one geometry; lane text ticks on the readings cadence from the same sample; per-lane sr mirror sentences; aspect-driven presence, no timed entrance. Pending Justin's live gut-check.
