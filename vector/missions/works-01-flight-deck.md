...
**Now:** Annotation layer MERGED 2026-07-04 (PR #180, alongside PR #179 which relanded the #178 tuner stack that had stalled on its base branch): caliper arcs hug the wall at the three tracked stress concentrations with hairline leaders to per-lane readouts (ST 1-3: bearing off 12 o'clock, intensity, WATCH/QUIET), anchored by inverting the shader's own transform from the same clock and pure field model, so arc and render cannot disagree; presence is aspect-driven across the same regime as the shader's anchor guard (no timed entrance); the sr mirror speaks a sentence per lane. Justin's live gut-check passed same day ("renders fine"), and lane states and the readings line derive from one telemetry sample and one watch floor (pinned by test), so they cannot drift apart. Pre-builds the pointable anatomy phases 4-5 need; lane-vocabulary legibility parked in open questions. Phase 2 context: MERGED 2026-07-03 (PR #175) with same-day motion pass; deck chunk 51 of 350 KB gz. Next up: **Phase 3 — Synthetic Orientation + Vacuum Energy** (Roy reviews again at phase 5). Boot-feel tuning still open (Justin sleeping on it). Dev tuning: `dialkit@1.3.0` drives the field shader's 15 params as live dials (`src/works/flight-deck/dev/FieldTuner.tsx`); dev-only, absent from production; "log values" copies JSON to bake back into `FIELD_MOTION_DEFAULTS`.
